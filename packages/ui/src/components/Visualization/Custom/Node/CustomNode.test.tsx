...
describe('getDragAndDropDirection', () => {
  ...
  it('should return the forward when nodes are at the same level', () => {
    ...
    (whenLogNode.getId as jest.Mock).mockReturnValue('route');
    (otherwisePlaceholderNode.getId as jest.Mock).mockReturnValue('route1');
    const result = getDragAndDropDirection(whenLogNode, otherwisePlaceholderNode);
    expect(result).toBe('forward');
  });
});
