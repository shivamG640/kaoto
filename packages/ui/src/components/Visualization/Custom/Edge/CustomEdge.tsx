  ...
  > = useMemo(
    () => ({
      ...
      canDrop: (item, _monitor, _props) => {
        ...
        if (draggedVizNode.getNextNode() === followingVizNode || draggedVizNode.getPreviousNode() === precedingVizNode)
          return false;
    ...
    <g className="custom-edge" ref={dndDropRef}>
      <path
        ...
        className={clsx('custom-edge__background', {
          'custom-edge__background__dropTarget': dndDropProps.canDrop && dndDropProps.hover,
          'custom-edge__background__possibleDropTargets':
            dndDropProps.canDrop && dndDropProps.droppable && !dndDropProps.hover,
        })}
      />
      <path className="custom-edge__body" d={edgeDRef.current} />
      <ConnectorArrow
        isTarget={true}
        className="pf-topology__edge"
